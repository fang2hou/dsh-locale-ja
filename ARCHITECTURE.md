...
DeepSeek Harness (DSH) `0.1.0-rc.6`, supported on the `web` profile. Its Host
...
4. **Only `src/client/locale-extension.ts` may contact locale internals, through
   ...
   [ADR-0002](./docs/adr/0002-extend-the-locale-service-through-internal-fields.md).
   DSH `0.1.0-rc.6` has no public API for adding a selectable locale, so this
...
7. **Dictionaries must use the platform's own compile-time key unions whenever
   ...
   `permission.access`
   (`@deepseek-ai/dsh-client-ui-permission-presets@0.1.0-rc.6`), and
   `trajectory` (`@deepseek-ai/dsh-client-ui-trajectory@0.1.0-rc.6`), naming
