...
const conversation: LocaleDictOf<"conversation"> = {
  ...
  "placeholder.plan": "タスクを記述して計画を生成",
  "placeholder.default": "エージェントにメッセージを送信",
  "placeholder.unavailable": "セッションは利用できません",
  ...
  "image.dropBlocked": "現在画像を追加できません",
  "image.pending": "送信待ちの画像",
  "image.openOriginal": "原画を表示",
  "image.openOriginalLabel": "{label}、クリックして原画を表示",
  ...
  "image.preview": "原画プレビュー",
  "image.closePreview": "原画プレビューを閉じる",
  "image.serviceUnavailable": "画像読み込みサービスは利用できません",
  "image.unsupportedType": "PNG、JPG、WebP、GIF 形式の画像のみ対応しています",
  "image.tooMany": "1メッセージにつき最大 {count} 枚まで追加できます",
  "image.fileTooLarge": "1枚あたり {size} を超える画像は追加できません",
  ...
  "image.subagentUnsupported": "サブエージェントセッションでは画像はまだサポートされていません",
  "image.sendFailed":
  ...
  "settings.enter.title": "ビジー時の Enter キーの動作",
  ...
  "message.maxTokens.hint":
    "回答が切り詰められました。これまでの出力は会話に保持されています。「続けて」と送信すると、モデルが続きを出力します。",
  ...
  "ask.rowTitle": "質問",
  "ask.waiting": "回答待ち",
  "ask.cancelled": "キャンセルされました",
  "ask.interrupted": "中断されました",
  ...
  "json.truncated": "… 切り詰め済み、全体で {total} 文字",
  "clock.md": "{m}月{d}日",
...
const feedback: LocaleDictOf<"feedback"> = {
  ...
  "note.open": "コメントを追加",
  "note.placeholder": "この回答のどこが良い、または悪かったですか？（任意）",
  "note.save": "保存",
  "note.cancel": "キャンセル",
...
const sessionLogDownload: LocaleDictOf<"session-log-download"> = {
  ...
  "dialog.successTitle": "Sessionのダウンロードを開始しました",
  "dialog.successDescription": "ブラウザがSessionのZIPファイルをダウンロードしています。",
  "dialog.errorTitle": "Sessionのエクスポートに失敗しました",
  "dialog.close": "閉じる",
...
const settingsModels: LocaleDictOf<"settings.models"> = {
  ...
  deleteDescription:
    "{provider} を削除するとその設定が取り除かれます。使用している資格情報（ある場合）は別の場所で管理されるため保持されます。",
  deleteDescriptionWithCredential:
    "{provider} を削除すると、その設定と保存済みの API キーが取り除かれます。",
  ...
  readOnly: "現在のデプロイの設定ドキュメントは読み取り専用です。",
  ...
  keyPlaceholderNative: "API キーを入力、または環境認証を使用する場合は空欄のまま",
  keyStored: "設定済み — 新しい値を入力して置き換え",
  ...
  removeModel: "モデルを削除",
  modelsEmpty:
    "モデルセレクターに表示されるモデルはありません。カタログ外の ID は直接送信できます。",
  keyBlank: "API キーを入力してください。空欄の場合は保存済みのキーが維持されます。",
  keyBlankNew:
  ...
  modelCapacityInvalid: "容量は数値で、K または M のサフィックスを付けられます。",
  ...
  customRouteHint:
    "小文字で始まる識別子。リクエスト内でこのプロバイダーを一意に識別し、資格情報名の派生に使用されます。",
  ...
  welcomeBody:
    "DeepSeek Harness の現在のバージョン 0.1 は、Harness 開発者向けのテスト段階にあり、今後も継続的な改善と洗練が必要な部分が多くあります。皆様からのフィードバックをお待ちしております。DeepSeek Harness のコアプラグインおよび基本 API は、今後しばらくの間、急速に反復・進化していく予定です。\n\nオープンソースで、開かれ、再利用可能で、組み合わせ可能なインフラのうえに、世界中の開発者とともに知性の限界を探求できることを楽しみにしています。世界中の Harness 開発者の皆様の DSH プラグインエコシステムへの参加を歓迎します。",
  welcomeContinue: "続行",
...
const settingsAgentPreset: LocaleDictOf<"settings.agentPreset"> = {
  ...
  userTrust: "カスタム",
  seatHint: "次に開始するセッションで使用する Agent プリセット",
  headerHint: "このセッションで実行中の Agent プリセット（開始時に固定）",
  nav: "Agent プリセット",
  sectionIntro:
    "プリセットとは、セッションの Agent が実行するプラグイン構成（ツール、プロンプト、能力）です。既存のプリセットをコピーして自分用に編集するか、「クリエイトモード」で Agent に作成させることができます。",
  builtIn: "組み込み",
  setDefault: "デフォルトに設定",
  view: "表示",
  presetStandardName: "スタンダードモード",
  presetStandardDescription:
    "ファイル編集、Shell、ファイルおよびウェブ検索、Skills、計画、ゴール、サブエージェント、ワークフローをサポートする、すべて揃ったコーディング Agent です。",
  ...
  presetMinimalDescription:
    "永続化された bash と str_replace_editor の 2 ツールのみを備えたコーディング Agent です。",
  presetCordisName: "クリエイトモード",
  presetCordisDescription:
    "カスタム Agent preset の作成向け。スタンダードモードの全能力に加え、実行時インスペクト、プラグインの実験、preset 作成のガイダンスを提供します。",
  duplicate: "複製",
  ...
  displayName: "名称",
  displayNamePlaceholder: "セレクターに表示する名前（未設定時は識別子を使用）",
  inUse: "使用中",
  builtInGroup: "組み込み",
  customGroup: "カスタム",
  ...
  copyTitle: "プリセットを複製",
  copyIntro:
    "プリセット全体がこのマシンに複製されます。識別子がディレクトリ名になり、後から変更できません。それ以外の内容は、プリセット自身のファイルで直接編集してください。",
  create: "作成",
  creating: "作成しています…",
  creatorDraft: "「クリエイトモード」でカスタムプリセットを作成",
  ...
  idTaken: "この識別子はすでに使用されています。",
  deleteTitle: "このプリセットを削除しますか？",
  deleteDescription:
    "プリセットのディレクトリが削除されます。すでにこの上で実行中のセッションには影響しませんが、新規セッションでは選択できなくなります。",
  deleteConfirm: "削除",
  deleting: "削除しています…",
};
...
const cordis: LocaleDictOf<"cordis"> = {
  ...
  "panel.hint": "実行コントロールは左下、設定の上にある Cordis パネルにあります",
  ...
  "action.approvePlugin": "このプラグインの以降のバージョンを許可",
  "action.decline": "拒否",
  "action.run": "実行",
  "action.stop": "停止",
  "action.remove": "削除",
...
const settingsPlugins: LocaleDictOf<"settings.plugins"> = {
  ...
  discard: "変更を破棄",
  unsaved: "未保存",
  saveFailed: "このデプロイはこれらの値を受け付けませんでした。修正用に保持しています。",
  invalidNumber: "数値を入力してください。空欄の場合はデフォルト値が使用されます。",
  bashTitle: "ターミナル",
  bashDescription: "Agent が実行する各コマンドを制限します。",
  bashTimeoutMs: "コマンドタイムアウト（ミリ秒）",
  bashTimeoutMsHint: "1コマンドの最長実行時間。超過すると終了します。",
  ...
  webSearchApiKeyHint: "設定ファイルには書き込まれません。空欄の場合は現在のキーが維持されます。",
  webSearchApiKeySet: "キーが設定済みです。",
  webSearchApiKeyUnset: "キーが未設定です。設定するまで検索は利用できません。",
  webSearchBaseUrl: "エンドポイントURL",
  webSearchBaseUrlHint: "空欄の場合はプロバイダーのデフォルトURLが使用されます。",
  webSearchMaxUses: "1リクエストあたりの最大検索回数",
...
const subagent: LocaleDictOf<"subagent"> = {
  ...
  "activity.inactive": "現在非実行",
  "branch.collapse": "{label} の下位サブエージェントを折りたたむ",
  "branch.expand": "{label} の下位サブエージェントを展開",
  "count.total.one": "{count} 個のサブエージェント",
  "count.total.other": "{count} 個のサブエージェント",
  "count.running.one": "{count} 個のサブエージェントが実行中",
  "count.running.other": "{count} 個のサブエージェントが実行中",
...
const workspace: LocaleDictOf<"workspace"> = {
  ...
  "status.running": "実行中",
  "status.subagentsRunning.one": "{n} 個のサブエージェントが実行中",
  "status.subagentsRunning.other": "{n} 個のサブエージェントが実行中",
  "status.idle": "アイドル",
  "status.waitingApproval": "承認待ち",
  "status.planReview": "計画レビュー待ち",
