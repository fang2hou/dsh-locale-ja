...
const conversation: LocaleDictOf<"conversation"> = {
  ...
  "image.tooMany": "1メッセージにつき {count} 枚まで追加できます",
...
const settingsModels: LocaleDictOf<"settings.models"> = {
  ...
  readOnly: "現在のデプロイの設定ファイルは読み取り専用です。",
  ...
  customRouteHint:
    "小文字で始まる識別子。リクエスト内でこのプロバイダーを一意に識別し、認証情報名としても使用されます。",
  ...
  welcomeBody:
    "DeepSeek Harness の現在のバージョン 0.1 は、Harness 開発者向けのテスト段階にあり、今後も継続的な改善と洗練が必要な部分が多くあります。皆様からのフィードバックをお待ちしております。DeepSeek Harness のコアプラグインおよび基本 API は、今後しばらくの間、イテレーションを重ねながら急速に進化していく予定です。\n\nオープンソースで開かれ、再利用も組み合わせも自由なインフラのうえに、世界中の開発者とともに知性の限界を探求できることを楽しみにしています。世界中の Harness 開発者の皆様の DSH プラグインエコシステムへのご参加を歓迎します。",
  welcomeContinue: "続行",
...
const settingsPlugins: LocaleDictOf<"settings.plugins"> = {
  ...
  bashTitle: "ターミナル",
  bashDescription: "エージェントが実行する各コマンドを制限します。",
  ...
  agentLoopDescription: "エージェントがツール呼び出しをディスパッチする方法。",
  ...
  webSearchBaseUrlHint: "空欄の場合はプロバイダーのデフォルト URL が使用されます。",
